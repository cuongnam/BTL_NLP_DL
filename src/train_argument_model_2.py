...
from transformers import DataCollatorForTokenClassification, XLMRobertaTokenizerFast
...
class BKEEArgumentDataset(torch.utils.data.Dataset):
    def __init__(self, data_path, label2id, tokenizer=None, tokenizer_name="xlm-roberta-base", max_len=256):
        ...
        self.tokenizer = tokenizer if tokenizer is not None else XLMRobertaTokenizerFast.from_pretrained(tokenizer_name)
        if not self.tokenizer.is_fast:
            raise RuntimeError("Argument training requires a fast tokenizer.")
